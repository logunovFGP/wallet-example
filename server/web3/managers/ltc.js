...
class LtcManager {
  ...
  static async createAccount(password, username) {
    const result = (await axios.post(`${process.env.BLOCKCYPHER_LITECOIN_API_URL}/addrs?token=${process.env.BLOCKCYPHER_API_TOKEN}`)).data;
  ...
  async getBalance() {
    const result = (await axios.get(`${process.env.BLOCKCYPHER_LITECOIN_API_URL}/addrs/${this.address}/balance?token=${process.env.BLOCKCYPHER_API_TOKEN}`)).data;
  ...
  async sendTransaction(to, value, password, privateObject, user, mnemonic) {
    ...
    let tmptx;
    try {
      tmptx = (await axios.post(`${process.env.BLOCKCYPHER_LITECOIN_API_URL}/txs/new?token=${process.env.BLOCKCYPHER_API_TOKEN}`, body)).data;
    } catch (e) {
    ...
    const result = (await axios.post(`${process.env.BLOCKCYPHER_LITECOIN_API_URL}/txs/send?token=${process.env.BLOCKCYPHER_API_TOKEN}`, tmptx)).data;
    const event = Event.build({
      ...
      detailsLink: `${process.env.LTC_EXPLORER_PREFIX}${result.tx.hash}`,
    });
    await event.save();

    const pollInterval = setInterval(async () => {
      axios.get(`${process.env.BLOCKCYPHER_LITECOIN_API_URL}/txs/${result.tx.hash}?token=${process.env.BLOCKCYPHER_API_TOKEN}`)
        .then(response => {
